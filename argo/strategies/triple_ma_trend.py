...
class TripleMATrendConfig(ImportableActorConfig):
    """
    ...
    Attributes:
        ...
        medium_period: Medium EMA period defining the value zone and trend baseline (default: 30).
        slow_period: Slow EMA period defining the macro regime filter (default: 100).
        trade_size: Position order quantity in base units or contracts (default: 100.0).
        min_adx: Minimum ADX trend strength threshold required to permit entry (default: 25.0).
        max_rsi_entry: Maximum RSI allowed to ensure entry is not overbought (default: 70.0).
    """
    instrument_id: str
    bar_type: str
    fast_period: int = 10
    ...
    min_adx: float = 25.0
    max_rsi_entry: float = 70.0


class TripleMAContinuationStrategy(NautilusStrategy):
    """
    ...
    def on_start(self):
        self.log.info(f"Starting TripleMAContinuationStrategy for {self._instrument_id}")
    ...
    def _open_long(self, close: float, atr: float):
        self.log.info(f"[{self._instrument_id}] Triple MA Trend Alignment. Going long at {close:.2f}")
    ...
    def _manage_position(self, bar: Bar, atr: float):
        ...
        if self.fast_ema < self.medium_ema or close < self.medium_ema - (0.5 * atr):
            self.log.info(f"[{self._instrument_id}] Triple MA Trend Breakdown ({close:.2f}). Closing long.")
    ...
    def _update_forecast(self, current_price: float, atr: float):
        ...
        self.log.info(
            f"[{self._instrument_id}] TripleMA Forecast: VaR95=${m.var_95_amount:.2f} | CVaR95=${m.cvar_95_amount:.2f} | "
    ...
    def on_stop(self):
        self.log.info(f"Stopping TripleMAContinuationStrategy for {self._instrument_id}")
